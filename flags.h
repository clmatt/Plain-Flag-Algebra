...
std::unordered_map<std::string, int> firstIndex(const int n, const int numColors, const std::vector<Graph> &zeros) {
	...
	for(int i = n/2; i <= n-1; ++i) {
		...
		if(sizeOfFlag > 0) {
			...
			//Set flag arbitrarily
			std::vector<int> flag(sizeOfFlag);
			for(int j = 0; j < sizeOfFlag; ++j) {
				flag[j] = j;
			}
...
std::unordered_map<std::string, int> lastIndex(const int n, const int numColors, const std::vector<Graph> &zeros) {
	std::unordered_map<std::string, int> output;
	std::unordered_map<std::string, int> flags1 = firstIndex(n, numColors, zeros); //Need to make sure we consider the same flags as in firstIndex, as there is a choice, really just need this as a set of the domain
	std::unordered_map<std::string, int> flags2; //Need to say which flag we are on
			...
			for(auto G : allGraphs) {
				...
				do {			
					...
					do {
						...
						Gcopy.setFlag(sigma); //Annoying b/c it calls Nauty twice (3x?)
						Graph G1 = Gcopy.getFlag();
						
	
						if(flags1.find(G1.getCanonLabel()) != flags1.end()) { //Make sure same flags as index1
							std::string tempStr = Gcopy.getCanonLabel();
							cont = false;
							
							//Different graphs may have same label
							//But all graphs with same flag with have different label
							if(output.find(tempStr) == output.end()) {
								if(flags2.find(G1.getCanonLabel()) == flags2.end()) {
									output.insert(make_pair(tempStr,0));
									flags2.insert(make_pair(G1.getCanonLabel(),1));
								}
							
								else {
									output.insert(make_pair(tempStr,flags2.at(G1.getCanonLabel())));
									flags2.at(G1.getCanonLabel()) = flags2.at(G1.getCanonLabel()) + 1;
								}
							}
						}
					...
					if(cont) {
						std::cout << "Something went wrong in lastIndex or firstIndex." << std::endl << std::endl;
						throw std::exception();
					}
...
//Generate map for first index in A used in fast plain flag algebra
std::unordered_map<std::string, int> fastFirstIndex(const int n, const int numColors, const std::vector<Graph> &zeros) {
	std::unordered_map<std::string, int> output;
	int index = 0;
...
//Generate map for last two indices in A used in fast plain flag algebra
//Doesn't need to contain any info about the first index  (even though it is implicitly there)
//String = add universal vtx, with different colored edges to parts
std::unordered_map<std::string, int> fastLastIndex(const int n, const int numColors, const std::vector<Graph> &zeros) {
	std::unordered_map<std::string, int> output;
		...
		if(sizeOfFlag > 0) {
			...
			for(auto G : allGraphs) {
				...
				do {
					...
					//Add new edge which is connected in new color to flag
		...
		for(int i = 0; i < allGraphs.size(); ++i) {
			...
			for(int j = n/2; j <= n-1; ++j) {
				...
				if(sizeOfFlag > 0) {
					...
					Frac e = Frac(1,choose(n,j)*choose(j,sizeOfFlag)*factorial(sizeOfFlag));	
					...
					do { //Next Subset
						do { //Next Permutation
							...
							if(firstIndexMap.find(G1.getCanonLabel()) != firstIndexMap.end()) { //Make sure this is actually a flag we are considering 
								...
								do { //Y subset
									...
									Graph G3 = G.restriction(restriction3); //Labels a reminant of an old way of doing it, could probably come up with better name now, but I'm too lazy
									...
									std::string lastIndexString1 = G3.getCanonLabel();
									std::string lastIndexString2 = G3c.getCanonLabel();				
...
void fastPlainFlagAlgebra(std::vector<Graph> &f, int n, std::vector<Graph> &zeros, std::vector<Equation> &known, bool maximize = true) {
	...
	std::cout << "Generating first index map." << std::endl;
	std::unordered_map<std::string, int> firstIndexMap = fastFirstIndex(n,numColors,zeros);
	std::cout << std::endl;
	...
	std::cout << "Generating last index map." << std::endl;
	std::unordered_map<std::string, int> lastIndexMap = fastLastIndex(n,numColors,zeros);
	std::cout << std::endl;
