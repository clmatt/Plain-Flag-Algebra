	...
	for(int i = 3; i <= 5; ++i) {
		for(int j = i; j <= 10; ++j) {
			...
			for(int k = 0; k < 10000; ++k) {
				...
				numSubgraphsNoFlags(H,G);
			}
		}
	}
	auto start4=high_resolution_clock::now();
	auto duration2 = duration_cast<seconds>(start4 - start3);
	cout << "Running time of old subgraphs: " << duration2.count() << endl << endl;
